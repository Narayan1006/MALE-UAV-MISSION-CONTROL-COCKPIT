...
- If a judge asks "why does CHT rise when throttle increases" — answer:
  more throttle -> more fuel burned -> more heat generated -> thermal balance
  equation (heat_in - heat_out) raises CHT until a new equilibrium is reached.

Run this file directly to generate example mission CSVs.
"""
...
class EngineConstants:
    idle_rpm: float = 1730.0
    max_rpm: float = 2529.8
    tau_rpm: float = 3.2
    k_fuel: float = 0.012
    cht_rise_max: float = 125.4
    tau_cht: float = 42.0
    egt_base: float = 53.8
    egt_gain: float = 5.42
    oil_rise_max: float = 63.7
    tau_oil: float = 74.0
    pressure_base: float = 24.6
    pressure_slope: float = 47.1
    pressure_temp_penalty: float = 0.12
...
class EngineSimulator:
    ...
    def step(self, dt: float, throttle_cmd: float, altitude_target: float,
        ...
        # --- composite health index: weighted deviation from nominal healthy bands ---
        # Denominators are the "how far above normal is concerning" scale for each
        # channel — tune these against real bands once real/reference data is available.
